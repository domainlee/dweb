...
class AttrListMapper extends Base{
    ...
    public function update($productId,$type, $attr, $storeId){
        if(!$productId){
            return null;
        }
...
//        echo count($attr);die;
    ...
    {
        if(!$attr->getProductId() && !$attr->getStoreId()){
            return null;
        }
        /* @var $dbAdapter \Zend\Db\Adapter\Adapter */
        $dbAdapter = $this->getServiceLocator()->get('dbAdapter');
        ...
        if($attr->getProductId()){
            $select->where(['p.productId' => $attr->getProductId()]);
        }
        if($attr->getStoreId()){
            $select->where(['p.storeId' => $attr->getStoreId()]);
        }
