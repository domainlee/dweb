	...
	protected function init(){
		...
		foreach ($this->dataSet as $item) {
			...
			$rows[] = array(
					...
					array (
                        'type' => 'text',
                        'value' => '<a href="/admin/product/edit/'.$item->getId().($this->urlQuery ? '?'.$this->urlQuery:null).'">'. $item->getTitle().'</a><a target="_blank" href="http://'.$_SERVER['HTTP_HOST'].$item->getViewLink().'" class="hide_ fa fa-eye"></a>',
                        'htmlOptions'=> array('style'=>'vertical-align: middle;'),
                    ),
                    array(
                        'type' => 'text',
                        'value' => $item->getQuantity() ? $item->getQuantity() : '',
                    ...
                    array(
                        'type' => 'link',
                        'value' => '<input class="changeStatus" '.($item->getStatus() == 1 ? 'checked':null).' data-id="'.$item->getId().'" type="checkbox" data-plugin="switchery" data-color="#5fbeaa" data-size="small"/>',
                        'htmlOptions'=> array('style'=>'text-align: center;vertical-align: middle'),
                    ),
					array(
                        'htmlOptions'=> array('style'=>'text-align: center;vertical-align: middle;'),
