...
//        $uri = $this->getUrlQuery() ? '&'.$this->getUrlQuery():'';
        ...
        if($startNumber > 1) {
            $previousPage = $currentPage - 1;
            $pagingHtml .= "<li><a class='btn-default' href='?page=$previousPage$uri' title='Trang sau'><span>Prev</span></a></li>";
        }
        else{
            $pagingHtml .= "<li class='prev disabled'><a><span>Prev</span></a></li>";
        }

        $pagingHtml .= "<li><a>".$currentPage." / ".$lastPage."</a></li>";

        if($endNumber < $rowCount) {
            $nextPage = $currentPage + 1;
            $pagingHtml .= "<li><a class='btn-default' href='?page=$nextPage$uri' title='Trang trước'><span>Next</span></a></li>";
        }
        else{
            $pagingHtml .= "<li class='next disabled'><a title='Trang trước'><span>Next</span></a></li>";
        }

        $pagingHtml .= "</ul>".($this->getSetup() ? '<div class="btn-group pull-right">
                    <a class="btn btn-white waves-effect waves-light" data-toggle="modal" data-target="#global-admin-modal">Cài đặt <i class="fa fa-cog"></i></a>
                </div>':'')."</div>";
        return $pagingHtml;
    }
