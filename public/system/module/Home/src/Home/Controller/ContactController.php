...
//        echo $to;die;
        if($this->getRequest()->isPost()) {
            ...
            if($form->isValid($edit = 2)){
                ...
                $content = $this->getRequest()->getPost()->toArray()['content'];
                ...
                if(!empty($to)) {
                    ...
                    $body .= '<p style="color: #333;">'.$content.'</p>';
                    $body .= '<p>Xin cảm ơn!</p>';
                    $body .= '</div>';

                    $title = $name ? $name:'Thông tin liên hệ';
                    $subject = $name ? $name:'Thông tin liên hệ';
                ...
                $data = $form->getData();
                $price = $this->getRequest()->getPost()->toArray()['price'];
                $contact = new \Home\Model\Contact();
                $contact->exchangeArray($data);
                $contact->setName($name ? $name:'Khách cần tư vấn');
                $contact->setContent($price ? $content.'<br/>Ngân sách dự kiến: '.$price:$content);
                $contact->setStoreId($storeId);
                $contact->setCreatedDateTime(DateBase::getCurrentDateTime());
                $contactMapper->save($contact);
            } else {
