        ...
        $filter->add(array(
            ...
            'validators' => array(
                array(
                    ...
                    'options' => array(
                        'messages' => array(
                            'isEmpty' => 'Không tìm thấy mã giao diện này'
                        )
                    )
                ),
                array(
...
//
                if($this->get('storeClone')->getValue()){
                    $template = new \Admin\Model\Template();
                    $template->setDirectory($this->get('storeClone')->getValue());
                    $templateMapper = $this->getServiceLocator()->get('Admin\Model\TemplateMapper');
                    if(empty($templateMapper->get($template))){
                        $this->get('storeClone')->setMessages(['Không tìm thấy mã giao diện.']);
                        $isVaild = false;
                    }
                }
