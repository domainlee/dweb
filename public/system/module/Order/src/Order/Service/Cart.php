...
//        print_r($wishlists);die;
        ...
        if (count($wishlists)) {
            foreach ($wishlists as $w) {
                ...
                $product->addOption('priceOff', $w->getPriceOff());
...
//                $wistlist->addOption('mode', $mode);
                ...
                $wistlistOff = new WishList();
                $wistlistOff->setProductStoreId($p->getProductId());
                if ($this->getUser()) {
                    $wistlistOff->setUserId($this->getUser()->getId());
                } else {
                    $wistlistOff->setUserCookie($this->getCookie());
                }
                $wishlistMapper->get($wistlistOff);
                if($wistlistOff->getPriceOff()) {
                    $wistlist->setPriceOff($wistlistOff->getPriceOff());
                }
    ...
    public function checkQuantity($id)
    {
        ...
        $wistlist = new WishList();
        $wistlist->setProductStoreId((int)$id);
        ...
        $wishlistMapper->get($wistlist);

        $productQuantity = new \Product\Model\Product();
        $productQuantity->setId((int)$id);
        $productMapper->get($productQuantity);

        if($wistlist->getQuantity() >= $productQuantity->getQuantity()) {
            return true;
        }
        return false;
    }
...
//            $moneyDiscount += $product->getQuantity() * $product->getMoneyDiscount();
            $totalMoney += $product->getQuantity() * ($product->getPriceOld() ? $product->getPriceOld():$product->getPrice());
        }
        if ($type == 2) {
